...
static struct mem_page* mem_page_create(const mem_pool_t *pool) {
    ...
    if (page == NULL) {
        EventCb_TriggerSimple(POOL_EXPAND_FAILURE, OPERATE_FAILURE, \
                              NULL, 0, 1);
        return NULL;
    }
    ...
    EventCb_TriggerSimple(POOL_EXPAND_SUCCESS, OPERATE_SUCCESS, NULL \
                         , 0, 1);
    return page;
}
...
void *mem_pool_init(void) {
    ...
    if (pool == NULL)
    {
        EventCb_TriggerSimple(POOL_INIT_FAILURE, OPERATE_FAILURE, \
                             NULL, 0, 1);
        return NULL;
    }
    ...
    EventCb_TriggerSimple(POOL_INIT_SUCCESS, OPERATE_SUCCESS, NULL \
                         , 0, 1);
    return pool;
}
...
void mem_pool_free(mem_pool_t* pool, void *ptr) {
    ...
    {
        EventCb_TriggerSimple(POOL_DESTROY_FAILURE, PARA_INVALID, \
                             NULL, 0, 1);
        return;
    }
    ...
    EventCb_TriggerSimple(POOL_DESTROY_SUCCESS, OPERATE_SUCCESS, \
                         NULL, 0, 1);
}
...
void *mem_pool_alloc(mem_pool_t *pool) {
    if (pool == NULL)
    {
        EventCb_TriggerSimple(POOL_ALLOC_FAILURE, PARA_INVALID, \
                             NULL, 0, 1);
        return NULL;
    }
    if (pool -> free_list == NULL || pool -> curr_page \
        ...
        if (page == NULL)
        {
            EventCb_TriggerSimple(POOL_ALLOC_FAILURE, \
            OPERATE_FAILURE, NULL, 0, 1);
            return NULL;
        }
...
//直接覆盖free_list，保留余下的一两个块时间开销大（只会用循环嵌套解决）
    ...
    EventCb_TriggerSimple(POOL_ALLOC_SUCCESS, OPERATE_SUCCESS, \
                         curr_block, 0, 1);
...
void *mem_pool_alloc_continuous(mem_pool_t *pool, uint32_t bytes) {
    if(!pool || bytes == 0 || bytes > 4096U)
    {
        return NULL; // 超过单页上限直接失败
    }
    // 取出一整页4096字节连续内存
    uint8_t *page_buf = (uint8_t *)mem_page_create(pool);
    if(page_buf == NULL)
        return NULL;
    // 直接返回页起始地址，FIFO按需使用前N字节即可
    return page_buf;
}
...
void mem_pool_free_page(mem_pool_t* pool, void *page_ptr) {
    // 1. 入参合法性校验
    if (pool == NULL || page_ptr == NULL)
    {
        return;
    }
    // 2. 把页的起始地址当成一个大块，直接链回空闲链表
    mem_block_t* page_block = (mem_block_t*)page_ptr;
    page_block->next = pool->free_list;
    pool->free_list = page_block;
}